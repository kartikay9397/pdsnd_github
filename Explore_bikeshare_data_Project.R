...
ny_1 <-
  ny[, -c(8:9)]    # Here i removed the eighth and ninth column from table 'ny'
chi_1 <-
  chi[, -c(8:9)]  # Here i removed the eighth and ninth column from table 'chi'

# Now i have combined all the rows of the three tables
cities_data <- rbind(ny_2, wash, chi_2)
...
Start_station <- summary(cd$Start.Station)

head(Start_station, 1)
mean(Start_station)
median(Start_station)
...
End_station <- summary(cd$End.Station)
